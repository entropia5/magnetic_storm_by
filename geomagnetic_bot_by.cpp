...
string get_weather(string city) {
    ...
    try {
        ...
        string advice;
        if (current_temp <= -15) {
            advice = "Ох, на улице лютый мороз! Пожалуйста, одевайтесь как можно теплее.\nБерегите себя...🧊";
        } else if (current_temp <= 0) {
            advice = "На улице морозно, так что не забудьте надеть любимую шапку и теплые рукавицы.\nПостарайтесь, чтобы Вам было уютно и тепло!";
        } else if (current_temp <= 10) {
            advice = "На улице сейчас довольно зябко.\nОдевайтесь, чтобы не простудиться, сейчас - самая обманчивая погода.";
        } else if (current_temp <= 20) {
            advice = "Погода приятная, но ещё не совсем лето...";
        } else {
            advice = "Какая замечательная теплынь! Можно смело выходить в лёгкой одежде.\nНаслаждайтесь теплом...☀️";
        }
        ...
        if (desc.find("дождь") != string::npos || desc.find("морось") != string::npos) {
            ...
            advice += "\n\nСегодня временами идёт снежок...\n\nБудьте осторожны и внимательны, под снегом местами может быть скользко ❄️";
        }
