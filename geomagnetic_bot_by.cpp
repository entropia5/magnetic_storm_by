...
string get_weather(string city) {
    ...
    try {
        ...
        string advice;
        if (current_temp <= -15) {
            advice = "Ох, на улице лютый мороз! Пожалуйста, одевайтесь как можно теплее: термобелье, самая теплая куртка и обязательно шарф до самого носа:) Не стойте долго на месте, заходите погреться. Берегите себя...🧊";
        } else if (current_temp <= 0) {
            advice = "На улице морозно, так что не забудьте надеть любимую шапку и теплые рукавицы. Шарф тоже не будет лишним. Главное - чтобы вам было уютно и тепло!";
        } else if (current_temp <= 10) {
            advice = "На улице сейчас довольно зябко.  Самое время для теплого пальто или куртки. И не забудьте застегнуться, чтобы не продуло шею. Хорошего вам дня!🧥";
        } else if (current_temp <= 20) {
            advice = "Погода приятная, но еще не совсем лето. 🍃 Накиньте легкую куртку или свитшот - так Вам будет гораздо комфортнее во время прогулки.";
        } else {
            advice = "Какая замечательная теплынь!  Можно смело выходить в легкой одежде. Наслаждайтесь теплом...☀️";
        }
        ...
        if (desc.find("дождь") != string::npos || desc.find("морось") != string::npos) {
            ...
            advice += "\n\nСмотрите, на улице снежок! Будьте осторожны, под снегом местами может быть скользко. Будьте осторожны и внимательны... ❄️";
        }
        ...
        res += " **Небольшой совет для Вас:** " + advice;
        ...
        if (r.status_code == 200) {
            ...
            for (auto& update : data["result"]) {
                ...
                if (update.contains("message") && update["message"].contains("text")) {
                    ...
                    if (txt == "/start") {
                        send_styled_msg(cid, "Здравствуйте! Я Ваш заботливый помощник. Слежу за погодой и магнитными бурями по всей Республике. Постараемся не оставить без внимания даже самые маленькие городки нашей Родины.");
                    } else if (txt == "⚡️ Магнитные бури") {
                        send_styled_msg(cid, get_current_kp() + "\n\n" + get_daily_forecast());
                    } else if (txt == "☁️ Прогноз погоды") {
                        send_styled_msg(cid, "📍 Напишите название любого города или поселка Беларуси, а я подскажу, как лучше одеться:");
                    } else if (txt == "📖 Справка") {
                        string s = "📊 **Справка**\n\nЯ присылаю каждый день прогноз бурь в 09:00 по Минску...\n\n";
